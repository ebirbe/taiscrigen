...
public class SerialToGsm {
	...
	public SerialToGsm(String port) throws NoSuchPortException,
		 ...
		 * ": ALERTA: El Dispositivo de SMS no ha sido detectado.");
		 * System.exit(0); }
		 */
	}
	...
	private String sendAndRecv(String s, int timeout) {
		try {
			...
			MyLogger.escribirLog(this.getClass().getName(),
					"Env/Rec Exception " + e);
	...
	synchronized public String sendSms(String numToSend, String whatToSend) {
		...
		if (whatToSend.length() > 140) {
			MyLogger.escribirLog(this.getClass().getName(),
					"Mensaje muy largo: Tiene " + whatToSend.length()
							+ " caracteres.");
			return "ERROR";
		}
	...
	public String checkSms() {
		...
		MyLogger.escribirLog(this.getClass().getName(),
				"String to be decoded :" + str);

		/*
		 * ComputeSmsData sms = new ComputeSmsData(); sms.setRcvdPdu(str); //
		 * SMSCNum = new String(sms.getRcvdPduSMSC()); senderNum = new
		 * String(sms.getRcvdSenderNumber()); smsMsg = new
		 * String(sms.getRcvdPduTxt());
		 * 
