...
public class GPSListener {
	...
	public static void main(String[] args) {
		...
		if (args.length > 0) {
			if (args[0].equals("--config")) {
				...
				System.out
						.println("--config \tMuestra el dialogo de configuracion.");
				System.out.println();
			}
		} else {
			iniciarServidor();
	...
	private static void iniciarServidor() {
		MyLogger.escribirLog(GPSListener.class.getName(),
				"Puerto de Internet: " + config.getHostPort());
		MyLogger.escribirLog(GPSListener.class.getName(), "Dispositivo SMS: "
				+ config.getGsmDevice());
