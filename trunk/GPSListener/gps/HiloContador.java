...
public class HiloContador extends Thread {
	...
	public void run() {
		try {
			sleep(15000);
			if(gpsh.id.equals("")){
				MyLogger.escribirLog(this.getClass().getName(),gpsh.name  + " no ha reportado ID, Eliminando...");
				GPSServer.wp.lblStatus.setText(gpsh.name  + " no ha reportado ID, Eliminando...");
				GPSServer.removeConectados(gpsh.name, gpsh);
				gpsh.leer = false;
				gpsh.finalize();
			}else{
				MyLogger.escribirLog(this.getClass().getName(),gpsh.name  + " ya reporto ID, Continuando...");
